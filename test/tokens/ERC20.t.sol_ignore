...
contract ERC20Test is Test {
    ...
    function setUp() public {
        ...
        bytes memory args = bytes.concat(abi.encode("Token"), abi.encode("TKN"), abi.encode(DECIMALS));

        address mintableTokenAddress = HuffNeoDeployer.config()
            .with_deployer(deployer)
            .with_args(args)
            .deploy("test/tokens/mocks/ERC20MintableWrappers.huff");
        token = IMintableERC20(mintableTokenAddress);

        // Deploy the Mock ERC20
        address mockTokenAddress = HuffNeoDeployer.config()
            .with_deployer(deployer)
            .with_args(args)
            .deploy("test/tokens/mocks/ERC20Wrappers.huff");
    ...
    //     address owner = token.owner(); // TODO: Update when bug is fixed with HuffNeoDeployer address
